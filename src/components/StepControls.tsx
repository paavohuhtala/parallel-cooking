...
export function StartDialog() {
  ...
  const step = pendingStart ? index.steps.get(pendingStart) : undefined

  return (
    <AnimatePresence>
      {step && (
        <Modal key={step.id} label="Kuka ottaa tämän vaiheen?" onClose={cancelStart}>
          ...
          <div className={ui.modalCooks}>
            {state.cooks.map((cook) => (
              <button
                ...
                onClick={() => confirmStart(step.id, cook.id)}
              >
          ...
          <div className={ui.modalActions}>
            <button className={cx(ui.btn, ui.btnGhost)} onClick={() => confirmStart(step.id, null)}>
